let
    ...
    part[1] = sum(x[2] * i for (i, x) in enumerate(sort(zip(hands, bids),
       lt = (x, y) -> lt_hands(x[1], y[1]))))
    ...
    part[2] = sum(x[2] * i for (i, x) in enumerate(sort(zip(map(usejoker, hands), bids, hands),
       lt = (x, y) -> lt_hands(x[1], y[1], x[3], y[3]))))
